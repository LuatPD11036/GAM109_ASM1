...
public class ASM_MN : Singleton<ASM_MN>
{
    ...
    public void YC1()
    {
        ...
        Players player1 = new Players(id, "Luat", 500, new Region(2, "VN2"));
    ...
    public void YC4()
    {
        int currentPlayerId = listPlayer[0].Id;
        var findPlayerID = from plr in listPlayer
                           where plr.Id == currentPlayerId
                           select plr;
        Debug.Log("Sử dụng Linq để tìm Player theo Id:");
        foreach (var plr in findPlayerID)
        {
            string rank = calculate_rank(plr.Score);
            Debug.Log("Name: " + plr.Name + " - Score: " + plr.Score + " - Rank: " + rank);
        }
